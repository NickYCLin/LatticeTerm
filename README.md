...
若要讓另一個 CLI 接手，請在執行中的工作階段分頁選擇「加開 CLI」；來源為 Codex 或 Claude 時，可勾選「帶入目前對話」。新 CLI 會收到整理過的對話交接內容，但不會搬移模型內部狀態、登入資料或憑證。Codex 有明確 Session ID 時只接受 metadata 完全相符且不是 subagent 的 rollout；沒有 ID 時則只選同一 canonical 工作目錄的主 CLI 對話。Claude 也以 JSONL metadata 的 Session ID 精確尋找已驗證的主工作階段；沒有 ID 時才依 canonical 工作目錄選取，不依可能碰撞的專案資料夾 slug。兩者的有界歷程搜尋都不跟隨符號連結。若更換工作目錄需要跨 CLI 對話交接，所有對話會先成功匯出才啟動替代程序；任何一份匯出失敗都會取消整批更換並保留全部原工作階段。

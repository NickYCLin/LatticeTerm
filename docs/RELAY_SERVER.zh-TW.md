...
任何人都能送出連線與查找請求；真正建立工作階段仍需正確的32 位十六進位隨機配對碼與
...
- **被分享端**：「分享這台裝置」→ 分享方式選「透過中繼伺服器」，
  ...
  配對碼；配對碼可自訂為固定的 32 位十六進位隨機碼（無人值守用），或每次分享自動產生。
...
- **無人值守（固定配對碼）**：桌面內嵌分享不會保存固定碼，並以 stdin
  交給 Agent；獨立常駐服務應把32 位十六進位隨機碼放在只有服務帳號可讀的檔案，改用
  ...
  立即重啟並重設失敗次數。命令列 `--pair-code TOKEN` 可接受同樣的 32 位隨機碼供臨時手動
...
兩端跑不同的 LatticeTerm 版本仍可用裝置 ID 連線——**落後的那台必須連得進去，
否則沒有人能連上去幫它更新**。規則：

- 檢視端接受版本落在 `MIN_COMPATIBLE_PROTOCOL_VERSION` 到自己 `PROTOCOL_VERSION`
  之間的 Agent，並降到對方的版本溝通，不是要求完全相同。
- 對方比自己新則拒絕——無法猜測新版加了什麼。錯誤訊息會明講該更新哪一台，
  只說「協定不相容」等於讓人瞎猜。

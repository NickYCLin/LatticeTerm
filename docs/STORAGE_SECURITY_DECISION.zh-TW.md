...
- SSH／SFTP／RDP／VNC 密碼預設只供單次連線；只有使用者明確選擇記住且驗證成功後才存入選定認證後端。內嵌 Lattice Remote 直連碼只留在程序／WebView 記憶體並於配對後清除；中繼模式的固定碼同樣不持久化，且以 stdin 送入 sidecar，不出現在程序參數。獨立常駐 Agent 應改用權限限定為擁有者的 `--pair-code-file`。中繼位址屬非機密設定，可保存於前端；永久裝置身分含註冊 token 與 Noise 私鑰，Unix 上強制為 `0600`；該檔在首次讀取時才建立，因此只有在使用者於分享對話框選擇中繼模式後才會產生，從未使用 Lattice Remote 或只用區網直連的人不會被建立這份金鑰。RDP／VNC 密碼只以 stdin 傳入每工作階段獨立的 engine。

...
- 已完成敏感剪貼簿保護；Lattice Remote 配對碼預設複製 30 秒後清除，可選 15／60／120 秒或停用。Rust 原生層只保存 SHA-256 摘要並在逾時、使用者要求或應用程式離開時比對內容，相符才清除；WebView 不取得 clipboard plugin 的任意讀取權限。瀏覽器預覽使用相同摘要與比對規則。

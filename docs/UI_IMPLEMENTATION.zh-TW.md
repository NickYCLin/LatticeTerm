...
- 拖曳上傳走另一條路徑：Tauri 的 `onDragDropEvent` 只把本機檔案路徑交給目前作用中的 `SftpPane`，後端 `sftp_upload_path` 直接從磁碟串流到遠端，沿用同一套「先寫暫存檔、完成才改名、失敗清半成品」的保護；介面會合併非同步事件而不讓較舊的初始回應倒退進度，並在完成事件後重新整理目前目錄。目前僅支援單檔，資料夾會被擋下。

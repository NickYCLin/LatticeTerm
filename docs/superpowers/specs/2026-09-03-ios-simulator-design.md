...
- 交付包含已驗證的 Simulator build、Xcode 專案，以及使用指定 In-House 描述檔匯出的企業 IPA；尚未在實體裝置安裝驗證。App Store Connect、TestFlight、隱私權宣告與上架素材仍是後續獨立發布階段。

## 實作方式

1. 以 npm lockfile 安裝既有 Tauri CLI，安裝 Rust stable 與 Tauri 官方所列的 iOS targets。
2. 執行 `npx tauri ios init` 產生 Tauri 的 Apple/Xcode 專案，使用 In-House profile 對應的 bundle identifier `tw.nickyclin.latticeterm` 與 Team ID `SQDAQK66UY`。
3. 新增 `tauri.ios.conf.json`，與 Android 設定一樣將 `bundle.externalBin` 清空，保證 iOS build 不納入或嘗試執行桌面 sidecar。
4. 以 simulator target 完成 Tauri iOS build；再由 Tauri 開啟 Xcode，並以 Xcode build 驗證產生的 scheme。企業版先建立 arm64 archive，再以 `ExportOptions.plist` 的 enterprise 匯出設定產生 IPA。
...
- `xcodebuild -exportArchive` 必須以 In-House 描述檔成功匯出 IPA；其 bundle identifier、Team ID、Distribution 簽章與內嵌描述檔皆必須符合設定。
...
- Simulator 與 IPA 簽章檢查無法證明實機安裝、網路權限或 Keychain 行為。這些會明確記為未驗證項目，不宣稱已可 TestFlight 發布。

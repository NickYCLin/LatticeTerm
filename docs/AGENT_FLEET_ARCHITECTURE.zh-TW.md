...
| 工具專用語意 Adapter | 部分完成 | Codex `notify`、Claude Code、Gemini CLI、Qwen Code lifecycle hooks 與 OpenCode plugin events 已接上 Reporter；舊工作區續接 recipe 與保守的 session ID 擷取仍保留，其他工具 hook、token／cost 擷取尚未完成 |
...
Reporter 傳輸與狀態模型已完成，Codex、Claude Code、Gemini CLI、OpenCode 與 Qwen Code 也已有工作階段限定的完成 hook／plugin event。舊工作區相容層仍保留 Adapter v1 的四種原生 restore recipe，以及白名單 CLI 的保守 session ID 擷取，但目前介面不再提供手動續接區塊。下一步擴充其他工具的 hook 安裝方式，再加入 token／cost 等可觀測事件；未整合 CLI 繼續使用保守 heuristic，也可由工具 hook 呼叫通用 Reporter。

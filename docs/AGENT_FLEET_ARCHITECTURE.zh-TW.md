...
- 支援使用者明確勾選執行中的 Agent，經二次確認後將同一段提示送進最多 32 個獨立 PTY；每個目標逐一回報成功或失敗，提示內容不會保存。執行中分頁加開 CLI 時，也會詢問是否帶入目前脈絡；來源對話可讀時，目標 Claude 在其預設、可驗證的自動記憶設定下才更新專案 `MEMORY.md` 的 LatticeTerm 專屬區塊，保留使用者原有記憶。其他目標或未知格式一律只送一次性交接內容，不改寫私有 session 檔。

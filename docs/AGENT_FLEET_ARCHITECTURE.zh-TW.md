...
| 對話模式 | 已完成 | Claude Code、Codex 與 Gemini CLI 以官方 headless JSON 模式逐輪執行；串流文字、工具卡片、用量統計與以 CLI 對話 ID 續接；Claude（stream-json 控制協定）與 Codex（app-server JSON-RPC）支援逐項核准；Gemini 的非互動模式無對應機制 |
